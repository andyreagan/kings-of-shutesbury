...
from segments.models import Athlete, EffortLog, Effort, Segment
from segments import scoring
from conftest import insert_segment

pytestmark = pytest.mark.django_db


def log_effort(seg_id: int, athlete_id: int, elapsed: int,
               date: str = "2026-05-01T10:00:00Z") -> None:
    Athlete.objects.get_or_create(
        id=athlete_id,
        defaults={"name": f"Athlete {athlete_id}"}
    )
    EffortLog.objects.create(
        segment_id=seg_id,
        athlete_id=athlete_id,
        elapsed_time=elapsed,
        effort_id=f"e{athlete_id}",
        start_date_local=date,
        observed_at="2026-06-01T00:00:00+00:00",
    )


def view_ranks(seg_id: int) -> dict[int, int]:
    return {
        e.athlete_id: e.rank
        for e in Effort.objects.filter(segment_id=seg_id)
    }


def test_two_way_tie_shares_the_kom():
    ...
    next rider is 3rd."""
    insert_segment(1)
    log_effort(1, 101, 35, date="2014-04-29T16:48:26Z")
    log_effort(1, 102, 35, date="2026-05-20T17:44:02Z")
    log_effort(1, 103, 39)
    assert view_ranks(1) == {101: 1, 102: 1, 103: 3}


def test_mid_board_tie_skips_the_next_place():
    """Times 100, 110, 120, 120, 130 place 1, 2, 3, 3, 5."""
    insert_segment(2)
    for aid, t in [(201, 100), (202, 110), (203, 120), (204, 120), (205, 130)]:
        log_effort(2, aid, t)
    assert view_ranks(2) == {201: 1, 202: 2, 203: 3, 204: 3, 205: 5}


def test_tied_places_each_earn_full_points():
    ...
    board pays 9, 9, 2.)"""
    insert_segment(3, difficulty=100.0, total_efforts=1000)  # Cat 3, depth 8
    log_effort(3, 301, 35)
    log_effort(3, 302, 35)
    log_effort(3, 303, 39)
    ranks = view_ranks(3)
...
def test_changelog_ranks_are_competition_style():
    ...
    assert pipeline.competition_ranks(state) == {
