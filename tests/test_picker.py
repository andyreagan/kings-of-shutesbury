...
def test_empty_db_returns_none():
    assert pipeline._background_pick() is None


# === Phase A: stalest top-25 =================================================


def test_phase_a_picks_never_refreshed_first(seed_segment):
...
def test_phase_a_picks_oldest_when_no_nulls(seed_segment):
    ...
    sid, _, phase = pipeline._background_pick()
    assert sid == 1
    assert phase.startswith("A")


def test_phase_a_skips_fresh_segments(seed_segment):
...
def test_phase_b_picks_shallowest(seed_segment):
...
def test_phase_b_treats_null_as_depth_one(seed_segment):
...
def test_phase_b_skips_segments_at_max_depth(seed_segment):
...
def test_phase_c_when_all_fresh_and_at_max(seed_segment):
    fresh = _iso_days_ago(1)
    seed_segment(1, efforts_fetched_at=fresh,
...
def test_picker_skips_excluded(seed_segment):
    seed_segment(1, excluded=1)
    seed_segment(2)
    sid, _, _ = pipeline._background_pick()
    assert sid == 2


def test_picker_skips_non_ride(seed_segment):
...
def test_picker_skips_out_of_town(seed_segment):
...
def test_picker_skips_unimported(seed_segment):
